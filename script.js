...
const { ChevronDown, ChevronUp, LayoutDashboard, Dumbbell, Calendar, BarChart2 } = lucideReact;  
...
const threeYearData = Array.from({ length: 36 }).map((_, index) => {  
    ...
    const weekPlans = Array.from({ length: numberOfWeeks }).map((_, wIndex) => ({  
        ...
        days: [  
            { dayTitle: "胸 + 三头 + 有氧", machineUsed: "胸推机、龙门架绳索、跑步机", tips: "保持核心收紧，注意肘角度；有氧20-30分钟。" },  
            { dayTitle: "背 + 二头 + 有氧", machineUsed: "高位下拉、坐姿划船、椭圆机", tips: "挺胸收肩，集中背部发力；椭圆机15-20分钟。" },  
            { dayTitle: "腿 + 臀 + 宽距深蹲", machineUsed: "史密斯机、腿举机", tips: "背部挺直，核心紧绷，避免膝盖内扣。" },  
            { dayTitle: "肩 + 上肢综合", machineUsed: "哑铃推肩/侧平举、史密斯机耸肩", tips: "注意肩袖稳定，不要过度耸肩。" },  
            { dayTitle: "灵活安排/休息", machineUsed: "可做核心或拉伸放松", tips: "视个人状态而定，适度休息也重要。" }  
        ]  
    }));  

    const dailyMeals = Array.from({ length: numberOfDays }).map((_, dayIndex) => {  
        const dayNum = dayIndex + 1;  
        const lunchOptions = ["鸡胸肉 + 清炒西兰花 + 糙米饭", "牛肉片 + 白灼青菜 + 全麦饭"];  
        const dinnerOptions = ["三文鱼 + 蒜蓉菠菜 + 半碗糙米饭", "虾仁 + 生菜沙拉(低油) + 红薯"];  
        return {  
            dayLabel: `Day ${dayNum}`,  
            ...
            detail: "做法：保持均衡饮食，适量摄入蛋白质。"  
        };  
    });  

    const weeklyGroceries = Array.from({ length: numberOfWeeks }).map((_, wIndex) => ({  
        label: `第${wIndex + 1}周需购买：`,  
        items: ["鸡胸肉/牛肉/三文鱼", "西兰花/青菜/黄瓜", "糙米/全麦面包"]  
    }));  

    return { monthLabel, weekPlans, dailyMeals, weeklyGroceries };  
});  
...
function ThreeYearFitnessApp() {  
    ...
    return (  
        <div className="min-h-screen flex flex-col bg-gray-100">  
            <header className="bg-teal-700 text-white p-4 flex items-center justify-center">  
            ...
            <nav className="bg-white border-t border-gray-300 flex justify-around text-xs text-gray-500">  
                <TabButton label="概览" icon={<LayoutDashboard size={20} />} active={activeTab === "overview"} onClick={() => setActiveTab("overview")} />  
                <TabButton label="训练" icon={<Dumbbell size={20} />} active={activeTab === "training"} onClick={() => setActiveTab("training")} />  
                <TabButton label="饮食" icon={<Calendar size={20} />} active={activeTab === "diet"} onClick={() => setActiveTab("diet")} />  
                <TabButton label="进度" icon={<BarChart2 size={20} />} active={activeTab === "progress"} onClick={() => setActiveTab("progress")} />  
            </nav>  
        </div>  
    );  
}  
...
function TabButton({ label, icon, active, onClick }) {  
    return (  
        <button className={`flex flex-col items-center py-2 w-1/4 ${active ? "text-teal-700 font-semibold" : "text-gray-500"}`} onClick={onClick}>  
            {icon}  
...
function OverviewScreen() {  
    return (  
        <div className="p-3">  
            ...
            <p className="text-sm text-gray-700 leading-relaxed">在这里你可以总览三年的健身规划：通过底部导航可切换查看「训练」「饮食」「进度」等更详细的内容。</p>  
...
function TrainingScreen({ monthList, selectedMonth, setSelectedMonth, data, openSections, toggleSection }) {  
    return (  
        <div>  
            ...
            <select className="border rounded p-2 mb-4" value={selectedMonth} onChange={(e) => setSelectedMonth(Number(e.target.value))}>  
                {monthList.map((m) => (  
        ...
        <div>  
            ...
            <div className="bg-white rounded shadow-sm border border-gray-100 p-3 mb-4">  
                ...
                {data.dailyMeals.slice(0, 5).map((mealPreview, i) => (  
                    <p key={i} className="text-sm text-gray-600">{mealPreview.dayLabel}：{mealPreview.lunch} / {mealPreview.dinner}</p>  
                ))}  
                <p className="text-xs text-gray-400 mt-2">* 默认仅展示部分，可点击下方展开查看所有</p>  
            </div>  
            <CollapsibleSection title="展开查看每日餐单" sectionKey="diet-dailyMeals" openSections={openSections} toggleSection={toggleSection}>  
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">  
                    {data.dailyMeals.map((meal, idx) => (  
                        <div key={idx} className="bg-gray-50 p-3 rounded flex flex-col">  
                            ...
                            <p className="text-xs text-gray-700 mt-1"><strong>午餐：</strong>{meal.lunch}</p>  
                            <p className="text-xs text-gray-700"><strong>晚餐：</strong>{meal.dinner}</p>  
                            <p className="text-[11px] text-gray-500 mt-1">{meal.detail}</p>  
                        </div>  
                    ))}  
                </div>  
            </CollapsibleSection>  
            <CollapsibleSection title="展开查看每周购物清单" sectionKey="diet-groceries" openSections={openSections} toggleSection={toggleSection}>  
                {data.weeklyGroceries.map((wg, i) => (  
                    <div key={i} className="mb-3 bg-gray-50 rounded p-3">  
                        ...
                        <ul className="list-disc list-inside text-sm text-gray-700">  
                            {wg.items.map((item, ii) => <li key={ii}>{item}</li>)}  
                        </ul>  
                    </div>  
                ))}  
            </CollapsibleSection>  
...
// 进度(Progress) Tab 页面  
function ProgressScreen({ weightRecords, todayWeight, setTodayWeight, onAddWeight, chartData }) {  
    return (  
        <div>  
            ...
            <div className="mb-4 bg-white border border-gray-100 rounded p-3 shadow-sm">  
                ...
                <div className="flex items-center mb-2">  
                    <input type="number" step="0.1" placeholder="如: 66.5" value={todayWeight} onChange={(e) => setTodayWeight(e.target.value)} className="flex-1 border border-gray-300 rounded-l px-3 py-1.5 focus:outline-none focus:ring-1 focus:ring-teal-300" />  
                    <button onClick={onAddWeight} className="bg-teal-500 text-white px-4 py-1.5 rounded-r font-medium hover:bg-teal-600 transition-colors">记录</button>  
                </div>  
            </div>  
            <CollapsibleSection title="查看历史体重记录" sectionKey="progress-weightList" openSections={{}} toggleSection={() => {}}>  
                <div className="max-h-40 overflow-y-auto text-sm text-gray-700 space-y-1 mt-2 px-1">  
                    {weightRecords.slice().reverse().map((r, idx) => (  
                        <div key={idx}>{r.date} - {r.weight} kg</div>  
                    ))}  
                </div>  
            </CollapsibleSection>  
            <div className="mt-4 bg-white rounded p-3 shadow-sm border border-gray-100">  
                ...
                <LineChart width={350} height={200} data={chartData} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>  
...
function CollapsibleSection({ title, sectionKey, openSections, toggleSection, children }) {  
    ...
    return (  
        <div className="mb-4 border border-gray-200 rounded p-2 bg-white shadow-sm">  
            <div className="flex items-center justify-between cursor-pointer" onClick={onTitleClick}>  
